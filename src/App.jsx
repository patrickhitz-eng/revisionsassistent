  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fafafe', fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <div style={{ width: 400, textAlign: 'center' }}>
        <div style={{ width: 48, height: 48, borderRadius: 14, background: 'linear-gradient(135deg, #7c3aed, #a855f7)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, color: 'white', fontWeight: 800, margin: '0 auto 24px', fontFamily: "'Syne'" }}>R</div>
        <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 8, color: '#0f172a', fontFamily: "'Syne'" }}>Code eingeben</h2>
        ...
        <div style={{ background: 'white', borderRadius: 12, border: '1px solid #e2e8f0', padding: '28px 24px' }}>
          <input
            ...
            autoFocus
            style={{
              width: '100%', padding: '14px 18px', border: '1px solid #e2e8f0', borderRadius: 8,
              fontSize: 24, textAlign: 'center', letterSpacing: '0.2em', fontFamily: "'JetBrains Mono'",
          ...
          <button
            ...
            style={{
              ...
              background: code.length >= 6 ? 'linear-gradient(135deg, #7c3aed, #a855f7)' : '#e2e8f0',
              color: code.length >= 6 ? 'white' : '#94a3b8',
              border: 'none', borderRadius: 8, fontSize: 15, fontWeight: 600, cursor: code.length >= 6 ? 'pointer' : 'not-allowed'
            }}
          >{verifying ? 'Wird geprüft...' : 'Anmelden'}</button>
        </div>
...
export default function App() {
  ...
  const [otpEmail, setOtpEmail] = useState(null) // shows OTP screen when set
  ...
  if (!session && otpEmail) {
    return <OtpScreen email={otpEmail} onVerify={handleVerifyOtp} onBack={() => setOtpEmail(null)} />
  }
