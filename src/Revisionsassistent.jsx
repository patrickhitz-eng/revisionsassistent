...
// ── SER Data ──────────────────────────────────────────────────────
const INDEP_CHECKS = ["Tatsächliche Unabhängigkeit (Art. 729 Abs. 1 OR)", "Keine Beeinträchtigung dem Anschein nach", "Keine Mitwirkung bei Buchführung, oder: getrennte Personen (Art. 729 Abs. 2 OR)", "Andere DL mit Selbstprüfungsrisiko dokumentiert/getrennt", "Persönliche Unabhängigkeit aller Teammitglieder", "Finanzielle Unabhängigkeit (keine Beteiligung/Darlehen)", "Keine verwandtschaftlichen Beziehungen zu Organen", "Rotationsvorschriften eingehalten"];
...
Beträge Ganzzahlen CHF. Aufwand negativ. Eigenkapital in liabilities. prior weglassen wenn nicht vorhanden.` }
  ...
  return parseJsonFromResponse(data, "PDF-Extraktion");
}
...

{
  ...
  "owners": "Eigentümerstruktur, sofern öffentlich bekannt",
  "relatedParties": "Organe (VR/GL-Mitglieder aus HR), Beteiligungen, nahestehende Personen",
  "accounting": "Vermutetes Rechnungswesen basierend auf Grösse/Branche",
  "keyRisks": "Wesentliche Geschäftsrisiken basierend auf Branche und Unternehmensprofil",
  "special": "Besonderheiten des Unternehmens",
  "changes": "Wesentliche Veränderungen, sonst 'Beim Mandanten zu erfragen'",
  ...
  const data = await r.json();
  if (data.error) throw new Error(data.error + (data.details ? ": " + data.details : ""));
  return parseJsonFromResponse(data, "Recherche");
}

// Helper: extract JSON from Claude API response (handles web search multi-block)
function parseJsonFromResponse(data, label) {
  ...
  throw new Error(`${label}: Konnte JSON nicht aus KI-Antwort extrahieren`);
}
...
Antworte NUR mit JSON, keine Backticks. Max 1 Satz Begründung pro Gebiet.
{"assessments":{"cash":{"level":"normal","reasoning":"..."},"receivables":{"level":"normal","reasoning":"..."},"inventory":{"level":"normal","reasoning":"..."},"fixedAssets":{"level":"normal","reasoning":"..."},"financialAssets":{"level":"normal","reasoning":"..."},"payables":{"level":"normal","reasoning":"..."},"interestDebt":{"level":"normal","reasoning":"..."},"provisions":{"level":"normal","reasoning":"..."},"accruals":{"level":"normal","reasoning":"..."},"equity":{"level":"normal","reasoning":"..."},"taxes":{"level":"normal","reasoning":"..."},"revenue":{"level":"normal","reasoning":"..."},"personnel":{"level":"normal","reasoning":"..."},"otherExp":{"level":"normal","reasoning":"..."},"finResult":{"level":"normal","reasoning":"..."},"extraord":{"level":"normal","reasoning":"..."},"appendix":{"level":"normal","reasoning":"..."}},"overallRisk":"2 Sätze","focusAreas":["key1","key2"],"goingConcernRisk":"1 Satz"}
Risikostufen: vernachlässigbar, normal, erhöht.` }]
    })
  });
  const data = await r.json();
  if (data.error) throw new Error(data.error + (data.details ? ": " + data.details : ""));
  return parseJsonFromResponse(data, "Risikobeurteilung");
}
...
// ── Mandantenübersicht ────────────────────────────────────────────
function MandantenScreen({ mandanten, onCreate, onSelect, user }) {
...
function JahreScreen({ mandant, onBack, onSelectYear, onCreateYear, onSettings }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <button onClick={onSettings} style={{ padding: "8px 16px", background: S.surface, color: S.muted, border: `1px solid ${S.border}`, borderRadius: 6, fontSize: 12, cursor: "pointer", fontFamily: S.font }}>⚙ Einstellungen</button>
      </div>
      <div style={crd()}>
      ...
      {mandant.years?.map((y, i) => (
        <div key={i} onClick={() => onSelectYear(y)} style={{ ...crd(), cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }} onMouseEnter={e => e.currentTarget.style.borderColor = S.blue} onMouseLeave={e => e.currentTarget.style.borderColor = S.border}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ padding: "3px 10px", borderRadius: 4, fontSize: 11, fontWeight: 600, background: y.status === "abgeschlossen" ? S.greenL : y.status === "archiviert" ? "#f1f5f9" : S.blueL, color: y.status === "abgeschlossen" ? S.green : y.status === "archiviert" ? S.hint : S.blue }}>{(y.status || "in_bearbeitung").replace("_", " ")}</span>
  ...
  const sendDocEmail = async (type, email, setSignature) => {
    ...
    try {
      const r = await fetch("/api/send-email", {
        ...
        body: JSON.stringify({ type, to: email, mandant: mandant.name, year: year.year, user, honorar, honorarFolge }),
      });
      const data = await r.json();
      if (data.success) {
        setSignature({ email, timestamp: data.timestamp || new Date().toISOString(), ip: "via Resend", hash: "ID:" + (data.emailId || "").slice(0, 20) });
...
export default function Revisionsassistent({ user, userId, onLogout }) {
  ...
  const loadMandanten = async () => {
    setDbLoading(true);
    const { data } = await supabase.from('mandanten').select('*, team_members(*), revision_years(*)').order('created_at', { ascending: false });
    if (data) setMandanten(data.map(m => ({ ...m, years: m.revision_years || [], team: m.team_members || [] })));
    setDbLoading(false);
  };

  const createMandant = async (m) => {
    const { data, error } = await supabase.from('mandanten').insert({ owner_id: userId, name: m.name, website: m.website }).select().single();
    if (!error && data) setMandanten(prev => [{ ...data, years: [], team: [] }, ...prev]);
    else if (error) alert('Fehler: ' + error.message);
  };

  const updateMandant = async (updated) => {
    const { error } = await supabase.from('mandanten').update({ name: updated.name, website: updated.website, mandatsleiter_email: updated.mandatsleiter_email, company_info: updated.company_info }).eq('id', updated.id);
    if (!error) {
  ...
  const createYear = async (mandantId, y) => {
    const { data, error } = await supabase.from('revision_years').insert({ mandant_id: mandantId, year: y.year, audit_type: y.type }).select().single();
    if (!error && data) {
      const updated = mandanten.map(m => m.id === mandantId ? { ...m, years: [...(m.years || []), data] } : m);
  ...
  const inviteTeamMember = async (mandantId, email) => {
    const { data, error } = await supabase.from('team_members').insert({ mandant_id: mandantId, email, role: 'assistent', invited_by: userId }).select().single();
    if (!error && data) {
      const updated = mandanten.map(m => m.id === mandantId ? { ...m, team: [...(m.team || []), data] } : m);
      setMandanten(updated);
      const sel = updated.find(m => m.id === mandantId);
  ...
  const handleHilferuf = async () => {
    ...
    try {
      const r = await fetch("/api/send-email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "hilferuf", to: ml, mandant: selMandant.name, year: selYear?.year, user }) });
      const data = await r.json();
    ...
    <div style={{ fontFamily: S.font, color: S.slate, minHeight: "100vh", background: S.bg }}>
      ...
      {view === "jahre" && selMandant && <div style={{ padding: "24px 32px", maxWidth: 900, margin: "0 auto" }}>
        <JahreScreen mandant={selMandant}
          ...
          onSettings={() => { setSettingsMandant(selMandant); setView("settings"); }}
        />
      </div>}
